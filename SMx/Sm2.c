...
#define LOHALF(x) ((uint)((x) & 0xffff))
#define HIHALF(x) ((uint)((x) >> 16 & 0xffff))
#define TOHIGH(x) ((uint)((x) << 16))
...
{
	uint w[DIG_LEN], carry, borrow, sum;
	int i, flag;
...
{
	uint u[DIG_LEN], v[DIG_LEN], a[DIG_LEN] = { 1 }, c[DIG_LEN] = { 0 };
	uint sum, carry;
	int i, flagu, flag;
...
void modorder(big r, big x)
{
	...
	uint x0, x1, y0, y1;
	uint s1, s2, s3, s4;
	uint *q1, q2[DIG_LEN + 1], clone[DIG_LEN + 2];
	uint borrow, sum;
	int i, k, flag;
...
{
	int i, flag;
	uint t1[DIG_LEN], t2[DIG_LEN], lambda[DIG_LEN];
...
void projpointdouble(point p, point q)
{
	uint t1[DIG_LEN], t2[DIG_LEN], t3[DIG_LEN], t4[DIG_LEN], t5[DIG_LEN];
	int i, flag1, flag2;
...
void mixpointadd(point r, point p, epoint q)
{
	uint t1[DIG_LEN], t2[DIG_LEN], t3[DIG_LEN], t4[DIG_LEN];
	uint t5[DIG_LEN], t6[DIG_LEN];
	uint sum, carry;
	int i, flag1, flag2;
...
{
	uint t1[DIG_LEN], t2[DIG_LEN], t3[DIG_LEN], t4[DIG_LEN];
	uint t5[DIG_LEN], t6[DIG_LEN], t7[DIG_LEN];
	uint sum, carry;
	int i, flag1, flag2;
...
{
	int i;
	uint g[DIG_LEN], h[DIG_LEN], l[DIG_LEN], m;
            ...
            unsigned char *sign, unsigned int sign_len)
{
	...
	modadd(y, m, prikey, (uint*)N);
	modinv(h, y, (uint*)N);
	mulmodorder(y, r, prikey);
	modsub(l, rand, y, (uint*)N);
	mulmodorder(s, h, l);
